...
function LoadingCard({ company, focusArea }) {
  ...
  const { pct, stageLabel } = useProgress(true);
  return (
    <div style={card}>
      <div style={{ display:"flex", alignItems:"center", gap:"9px", marginBottom:"20px" }}>
        <span style={{ fontSize:"14px" }}>{company.emoji}</span>
        <div>
          <div style={{ fontFamily:"'Playfair Display',serif", fontSize:"15px", color:"#e0e0e0", fontWeight:600 }}>{company.name}</div>
...
const card = { background:"#0d0d0d", border:"1px solid #191919", borderRadius:"8px", padding:"17px", transition:"border-color 0.2s" };
...
function DeepModal({ company, focusArea, onClose }) {
  ...
  const ac = accent(company);
  const { pct, stageLabel } = useProgress(state === "loading");
  ...
  return (
    <div onClick={onClose} style={{ position:"fixed", inset:0, background:"rgba(0,0,0,0.88)", display:"flex", alignItems:"center", justifyContent:"center", zIndex:999, padding:"24px" }}>
      <div onClick={e=>e.stopPropagation()} style={{ background:"#0c0c0c", border:"1px solid #222", borderRadius:"10px", padding:"28px", maxWidth:"600px", width:"100%", maxHeight:"82vh", overflowY:"auto", boxShadow:`0 0 80px ${ac}12` }}>
  ...
  const runSearch = async () => {
    ...
    for (let i = 0; i < toFetch.length; i++) {
      ...
      fetchIntel(company, focusArea, false)
        ...
        .finally(() => { setLoadingSet(prev => { const n = new Set(prev); n.delete(key); return n; }); });
    }
      ...
      <div style={{ borderBottom:"1px solid #131313", padding:"22px 34px", display:"flex", justifyContent:"space-between", alignItems:"center" }}>
        <div>
          <div style={{ display:"flex", alignItems:"baseline", gap:"10px" }}>
            <h1 style={{ fontFamily:"'Playfair Display',serif", fontSize:"21px", color:"#f0f0f0", fontWeight:600, margin:0, letterSpacing:"-0.02em" }}>Customer Marketing Intel</h1>
          ...
          <p style={{ color:"#2e2e2e", fontSize:"10px", fontFamily:"'DM Mono',monospace", margin:"4px 0 0", letterSpacing:"0.04em" }}>Best-in-class SaaS customer marketing — advocacy · champions · community</p>
        </div>
              ...
              return (
                <div key={key} className="ic" style={{ animationDelay:`${i*0.04}s` }}>
                  {isLoading
                    ...
                    : <IntelCard company={company} focusArea={focusArea} data={result?.data} error={result?.error} onDrillDown={() => result?.data && setModal({ company, focusArea })} />
                  }
                </div>
              );
            })}
