...
// Processes one company×focus at a time to stay within Vercel's 60s limit
...
export default async function handler(req, res) {
  ...
  // Get company + focus from query params for single-item mode
  // e.g. /api/refresh?company=notion&focus=advocacy
  // If not provided, picks a random uncompleted one
  ...
  // Send digest if we processed everything
  if (!companyId && !focusId) {
